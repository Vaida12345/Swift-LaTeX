...
public struct LargeOperator<LowerBound: LaTeXComponent, UpperBound: LaTeXComponent, Body: LaTeXComponent>: LaTeXComponent {
    ...
    public init<Variable: LaTeXComponent, T: LaTeXComponent>(_ operator: LargeOperatorOperator, _ variable: Variable, from lowerBound: T, to upperBound: UpperBound, body: (_ variable: Variable) -> Body) where LowerBound == BinaryComponent<Variable, LaTeXSymbols.BinaryRelation, T> {
        self.init(`operator`, lowerBound: BinaryComponent(lhs: variable, operator: LaTeXSymbols.BinaryRelation.equal, rhs: lowerBound), upperBound: upperBound, body: body(variable))
    }
